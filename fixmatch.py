...
# from model.semseg.unet import Projector
...
def main():
    ...
    if os.path.exists(os.path.join(args.save_path, 'latest501.pth')):
        checkpoint = torch.load(os.path.join(args.save_path, 'latest501.pth'))
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        if rank == 0:
            ...
            torch.save(checkpoint, os.path.join(args.save_path, 'latest501.pth'))
            if is_best:
                torch.save(checkpoint, os.path.join(args.save_path, 'best501.pth'))
